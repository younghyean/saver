    ...
    return new Promise((fulfill, reject) => {
      ...
      if(!(req.body.title)) 
        res.status(403).send({ message: 'please input all '});
          ...
          const query = 'insert into etc_comment set ?'; 
         let record = {
           etc_num: req.params.etc_num,
        ...
        connection.query(query, record, err => {
           if(err) res.status(500).send({ message: "inserting etc_comment error: "+err});
