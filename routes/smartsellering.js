    ...
    return new Promise((fulfill, reject) => {
      ...
      if(!(req.body.title)) 
        res.status(403).send({ message: 'please input all '});
          ...
          const query = 'insert into smart_comment set ?'; 
         let record = {
           smart_num: req.params.smart_num,
        ...
        connection.query(query, record, err => {
           if(err) res.status(500).send({ message: "inserting smart_comment error: "+err});
