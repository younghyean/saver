...
router.get('/:utilsearch', async function (req, res) {
    try {
        ...
        pool.getConnection();
        let query = `select utilize.util_num as num,utilize.category as category,user.id as id,utilize.title as title,utilize.kind as kind,utilize.product as product,utilize.price as price,utilize.period as period,utilize.time as time, 
...
order by utilize.util_num desc;`;
        ...
        res.status(500).send({message: 'selecting util error' + err});
    }
    finally {
