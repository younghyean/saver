...
router.get('/:search', async function (req, res) {
    ...
    let data=await connection.query(query,'%' + req.params.search + '%');
     let query1 =`select brand.brand_num as num,brand.category as category,user.id as id,brand.title as title,brand.kind as kind,brand.product as product,brand.price as price,brand.period as period,brand.time as time, 
...
order by brand.brand_num desc;`;
      let data1=await connection.query(query1,'%' + req.params.search + '%');
        ...
        let query2 =`select etc.etc_num as num,etc.category as category,user.id as id,etc.title as title,etc.kind as kind,etc.product as product,etc.price as price,etc.period as period,etc.time as time, 
...
order by etc.etc_num desc;`;
      ...
      let query3 =`select smart.smart_num as num,smart.category as category,user.id as id,smart.title as title,smart.kind as kind,smart.product as product,smart.price as price,smart.period as period,smart.time as time, 
...
order by smart.smart_num desc;`;
      ...
      let query4 =`select utilize.util_num as num,utilize.category as category,user.id as id,utilize.title as title,utilize.kind as kind,utilize.product as product,utilize.price as price,utilize.period as period,utilize.time as time, 
...
res.status(200).send({ result:array, message: 'ok' }); 
    ...
    res.status(500).send({ message: 'selecting category error' + err });
}
finally {
